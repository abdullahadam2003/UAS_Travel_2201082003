        ...
        <div class="card-body">
            <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="password" class="form-control" id="password" placeholder="{{ __('Password') }}" name="password" required />
                </div>
                <div class="form-group">
                    <label for="roles">{{ __('Role') }}</label>
                    <select name="roles[]" id="roles" class="form-control select-multiple" multiple="multiple" required>
                        @foreach($rolesList as $id => $role)
                            <option value="{{ $id }}" {{ (in_array($id, old('roles', $user->roles->pluck('id')->toArray())) || isset($user) && $user->roles->contains($id)) ? 'selected' : '' }}>{{ $role }}</option>
                        @endforeach
                    </select>
                </div>
